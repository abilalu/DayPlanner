...
var a= moment().format('dddd MMMM Do YYYY, h:mm');
  ...
  var row = ""
    ...
    for (var i= 6 ; i<=18; i++){
      ...
      col3 = $(`<div class ="col-lg-2"><button data-id="${i}" id="savePlanner" class="btn btn-success btn-block"><i class="fas fa-save"></i> Create Event</button></div>`)
      row.append(col1)
      row.append(col2)
      row.append(col3)
  ...
  //   Am to Pm time zone
   ...
   function updateColor(){
     ...
     for (var i= 6 ; i<=18 ;i++){
       console.log(hour,i)
       if(hour==i ) {
        $(`#inputText${i}`).css("background","orange")
       }else  if(hour<i ){
        
         $(`#inputText${i}`).css("background","lightyellow")
